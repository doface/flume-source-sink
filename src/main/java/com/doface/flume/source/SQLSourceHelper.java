 ...
 * <tt>type: </tt> com.doface.flume.source.SQLSource
 * <p>
...
public class SQLSourceHelper {
	...
	private String statusFilePath, statusFileName, connectionURL, table, selectFields, lastField, lastFieldIndex,
	...
	/**
	 * Builds an SQLSourceHelper containing the configuration parameters and usefull
	 * utils for SQL Source
	 *
	 * @param context
	 *            Flume source context, contains the properties from configuration
	 *            file
	 * @param sourceName
	...
	public SQLSourceHelper(Context context, String sourceName) {
		...
		selectFields = context.getString("select_fields");
		lastField = context.getString("lastfield", "");
		String[] fields = selectFields.split(",");
		for (int i = 0; i < fields.length; i++) {
			if (lastField.equals(fields[i])) {
				lastFieldIndex = String.valueOf(i);
				break;
			}
		}
		...
		for (int i = 0; i < queryResult.size(); i++) {
			...
			for (int j = 0; j < fields.length; j++) {
				if(rawRow.get(j) == null){
					...
					map.put(fields[j], rawRow.get(j).toString());
				}
			}
			row[0] = table + "$$$$$" + JSON.toString(map);
			allRows.add(row);
		}
		return allRows;
	}
...
//			System.out.println("FileComment===========" + statusFileJsonMap.get(QUERY_STATUS_FILE));
//			System.out.println("QuerySQL==============" + customQuery);
	...
	public String getLastFieldIndex() {
